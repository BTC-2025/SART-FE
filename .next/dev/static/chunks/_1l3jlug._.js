...
function Home() {
    ...
    <header class="web-navbar">
      <div style="display: flex; align-items: center; gap: 16px;">
        <div class="nav-brand" onclick="switchTab('home')">
          <i class="fa-solid fa-compass-drafting brand-icon"></i>
          <span>SART</span>
          <span class="brand-badge">UNIVERSAL</span>
        </div>
        
        <!-- Location badge moved near logo -->
        <div class="location-nav-btn" onclick="requestLocation()" style="flex-shrink: 0; align-self: center;">
          <i class="fa-solid fa-location-dot"></i>
          <span id="nav-location-txt">Indiranagar, Bengaluru</span>
          <i class="fa-solid fa-chevron-down" style="font-size: 8px;"></i>
        </div>
      </div>
      
      <!-- Center Links (with AI and Wallet options removed) -->
      <nav class="nav-links">
        <div class="nav-link active" id="nav-btn-home" onclick="switchTab('home')">
          <span>Home</span>
        </div>
        <div class="nav-link" id="nav-btn-store" onclick="switchTab('store')">
          <span>Store</span>
        </div>
        <div class="nav-link" id="nav-btn-bookings" onclick="openModal('modal-bookings-registry')">
          <span>Booking</span>
        </div>
      </nav>
      ...
      <div class="nav-controls">
        <!-- Wallet quick display (points removed) -->
        <div class="wallet-badge" onclick="switchTab('wallet')">
          <i class="fa-solid fa-wallet"></i>
          <span class="wallet-val" id="nav-wallet-balance">₹15,000.00</span>
        </div>
        
        <!-- Search icon button (Bokspot Style) -->
        <div class="ctrl-btn" onclick="openModal('modal-search')">
          <i class="fa-solid fa-magnifying-glass"></i>
        </div>
        
        <!-- Liked routes icon button (Bokspot Style) -->
        <div class="ctrl-btn" onclick="openModal('modal-liked')">
          <i class="fa-solid fa-heart"></i>
        </div>

        <!-- Profile icon button -->
        <div class="ctrl-btn" onclick="switchTab('profile')">
          <i class="fa-solid fa-user-gear"></i>
        </div>
        
        <!-- Bookings Registry icon button (Bokspot Style) -->
        <div class="ctrl-btn" onclick="openModal('modal-bookings-registry')">
          <i class="fa-solid fa-bag-shopping"></i>
        </div>
        
        <!-- Profile Dropdown -->
        <div class="profile-nav-btn" onclick="toggleProfileDropdown(event)" style="position: relative;">
        ...
        <div class="profile-dropdown-menu" id="profile-dropdown-menu">
          <div class="dropdown-header">
            ...
            <strong class="val">alex.carter@sart.com</strong>
            <span class="flag">🇮🇳 India</span>
          </div>
          
          <div class="dropdown-divider"></div>
          
          <a class="dropdown-item" onclick="switchTab('profile'); closeProfileDropdown();">
          ...
          <div class="dropdown-item-meta">
            <span class="label"><i class="fa-solid fa-coins"></i> Currency</span>
            <span class="value">INR (₹)</span>
          </div>
          <div class="dropdown-item-meta">
            <span class="label"><i class="fa-solid fa-flag"></i> Country</span>
            <span class="value">India</span>
          </div>
          <div class="dropdown-item-meta">
            <span class="label"><i class="fa-solid fa-shield-halved"></i> Beta Trust</span>
            <span class="value" style="color: var(--primary);">85%</span>
          </div>
          
          <div class="dropdown-divider"></div>
          
          <div class="theme-switch-container">
            <span class="theme-lbl">DISPLAY THEME</span>
            <div class="theme-toggles">
              <button class="theme-toggle-btn" id="theme-btn-light" onclick="setAppTheme('light')">LIGHT</button>
              <button class="theme-toggle-btn active" id="theme-btn-dark" onclick="setAppTheme('dark')">DARK</button>
              <button class="theme-toggle-btn" id="theme-btn-system" onclick="setAppTheme('system')">SYSTEM</button>
            </div>
          </div>
          
          <div class="dropdown-divider"></div>
          
          <a class="dropdown-item signout-btn" onclick="alert('Signed out successfully.'); closeProfileDropdown();">
            <i class="fa-solid fa-arrow-right-from-bracket"></i> Sign out
          </a>
        </div>

        <!-- BIT Tool Button (Custom dynamic SVG logo shape) -->
        <button class="bit-nav-btn" onclick="openModal('modal-bit-feed')" style="background: transparent !important; box-shadow: none !important; border: 1.5px solid var(--primary) !important; color: var(--primary) !important;">
          <svg class="bit-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: inline-block; vertical-align: middle;">
            <circle cx="5" cy="4" r="2" fill="currentColor"/>
            <rect x="3" y="8" width="4" height="12" rx="2" fill="currentColor"/>
            <rect x="10" y="4" width="4" height="16" rx="2" fill="currentColor"/>
            <rect x="17" y="4" width="4" height="12" rx="2" fill="currentColor"/>
            <circle cx="19" cy="20" r="2" fill="currentColor"/>
          </svg>
          <span style="margin-left: 6px;">BIT Tool</span>
        </button>
      </div>
    </header>
    
    <!-- SUB-NAVIGATION BAR (Bokspot Style) -->
    <div class="web-subnavbar">
      <div class="subnav-links">
        <a class="subnav-link" onclick="openModal('modal-air')">
          <i class="fa-solid fa-plane"></i> Travel
        </a>
        <a class="subnav-link" onclick="openModal('modal-rental')">
          <i class="fa-solid fa-hotel"></i> Stay & Accommodation
        </a>
        <a class="subnav-link" onclick="switchTab('store')">
          <i class="fa-solid fa-film"></i> Entertainment
        </a>
        <a class="subnav-link" onclick="openModal('modal-parking')">
          <i class="fa-solid fa-football"></i> Sports
        </a>
        <a class="subnav-link" onclick="openModal('modal-mechanic')">
          <i class="fa-solid fa-award"></i> Lifestyle Services
        </a>
      </div>
      <div class="subnav-actions">
        <button class="subnav-btn" onclick="switchTab('home')">
          <i class="fa-solid fa-chart-pie"></i> DASHBOARD
        </button>
        <button class="subnav-btn btn-vendor" onclick="alert('SART Vendor Registration Panel loading...')">
  ...
  </div><!-- End Web App Layout -->

  <!-- ==============================================
       OVERLAY MODALS (Centered Card Sheets)
       ============================================== -->
  
  <!-- 1. Ride (Road Vehicle) Modal -->
      ...
      <div class="form-group">
        ...
        <div class="option-select-grid">
          ...
          <div class="option-select-card" onclick="selectBookingOption(this, 'ride-tier', '1500')" data-val="Premium SUV">
            ...
            <span class="option-price">₹1,500.00</span>
          </div>
        </div>
      </div>
  ...
  <div class="modal-overlay" id="modal-travel-guide">
    <div class="modal-sheet centered-modal" style="max-width: 500px;">
      <div class="modal-header">
        <div class="modal-title"><i class="fa-solid fa-compass" style="color: var(--warning);"></i> Scenic Travel Guide</div>
        <div class="modal-close-btn" onclick="closeModal('modal-travel-guide')"><i class="fa-solid fa-circle-xmark"></i></div>
      </div>
      <div style="margin-top: 14px; display:flex; flex-direction:column; gap:12px;">
        <p style="font-size:12px; color:var(--text-secondary);">Unlock curated routes, travel highlights, and charging station spots along scenic pathways.</p>
        
        <div style="border: 1px solid var(--dark-border); padding: 12px; border-radius: 12px; background: rgba(251, 191, 36, 0.03); border-color: rgba(251,191,36,0.25);">
          <h4 style="font-size: 13.5px; display:flex; align-items:center; gap:6px; color:#fff;">
            <i class="fa-solid fa-route" style="color:var(--warning);"></i> Western Ghats Coastal Route (NH-66)
          </h4>
          <p style="font-size: 11px; color: var(--text-secondary); margin-top: 4px; line-height: 1.4;">
            Experience gorgeous driving lanes overlooking the Arabian Sea, featuring 6 high-speed EV charging stations and scenic seafood local shacks.
          </p>
          <button class="action-btn" style="padding:6px 12px; font-size:10px; width:auto; margin-top:8px;" onclick="loadScenicRouteNH66()">Plot Scenic Route</button>
        </div>

        <div style="border: 1px solid var(--dark-border); padding: 12px; border-radius: 12px; background: rgba(255,255,255,0.01);">
          <h4 style="font-size: 13.5px; display:flex; align-items:center; gap:6px; color:#fff;">
            <i class="fa-solid fa-water" style="color:var(--secondary);"></i> Mandwa Yacht Pier & Speedboats
          </h4>
          <p style="font-size: 11px; color: var(--text-secondary); margin-top: 4px; line-height: 1.4;">
            Hop on standard hovercraft speedboats or reserve private yacht charters from Mumbai Harbor straight to Alibaug Mandwa Jetty (20 minutes).
          </p>
          <button class="action-btn" style="padding:6px 12px; font-size:10px; width:auto; margin-top:8px;" onclick="closeModal('modal-travel-guide'); openModal('modal-sea');">Book Maritime Charter</button>
        </div>
      </div>
    </div>
  </div>
...
//# sourceMappingURL=_1l3jlug._.js.map